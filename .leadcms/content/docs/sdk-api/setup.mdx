...
updatedAt: '2026-03-05T11:58:33.202385Z'
title: SDK Setup Guide
description: >-
  Install and configure the LeadCMS SDK and CLI. Set up environment, initialize
  wizard, authenticate and pull initial content.
coverImageUrl: ''
coverImageAlt: ''
...
LEADCMS_COMMENTS_DIR=.leadcms/comments
LEADCMS_EMAIL_TEMPLATES_DIR=.leadcms/email-templates
LEADCMS_SETTINGS_DIR=.leadcms/settings

# Enable draft handling for local SDK reads in preview/dev workflows
LEADCMS_ENABLE_DRAFTS=false

# Optional preview override (`true` enables preview mode, `false` disables)
...
### Multi‑remote credentials

If you configure named remotes, keep API keys in remote-specific variables:

```bash
LEADCMS_REMOTE_PROD_API_KEY=your-production-key
LEADCMS_REMOTE_DEV_API_KEY=your-development-key
```

The generic `LEADCMS_API_KEY` remains a fallback for the default remote only.
...
For multi‑remote projects, use a `remotes` block and `defaultRemote`:
...
- **Directories** – confirm or change local folders for content, media, comments, email templates and settings.  Press Enter to accept the defaults.
...
If you choose a multi‑remote setup, the wizard can also create the initial `remotes` block and default remote for you.
...
<Callout type="info" title="Multiple CMS instances">
If you work with more than one CMS instance (e.g. production and development), configure named remotes and authenticate each one separately with `leadcms login -r <name>`.  See [Multi‑Remote Environments](/docs/sdk-api/multi-remote/) for the full setup guide.
</Callout>

## Download your content

Once your project is initialised, run a full sync with a single command:
...
This populates local directories with all supported entities available to your current access level (settings, content, media, comments, email templates).  The SDK stores incremental sync tokens so subsequent pulls only download changed items, making updates fast even for large projects.  For more details on pull operations see the [Pull Content](/docs/sdk-api/pull-content/) article.
...
<FeatureGrid columns={2}>
  ...
  <FeatureCard
    ...
    description="Synchronize local and remote email templates"
    href="/docs/sdk-api/email-templates/"
  />
  <FeatureCard
    title="Settings Sync"
    description="Manage tracked settings through CLI"
    href="/docs/sdk-api/settings-sync/"
  />
  <FeatureCard
    title="Multi‑Remote"
    description="Work with multiple CMS instances from one project"
