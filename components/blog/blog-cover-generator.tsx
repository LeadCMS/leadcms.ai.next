...
export function BlogCoverGenerator({
  ...
  const heightClass = size === "hero" ? "aspect-[16/9]" : "h-72 md:h-80"
  ...
  return (
    <div className={`relative w-full ${heightClass} overflow-hidden rounded-t-lg`}>
      ...
      {coverImageUrl ? (
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-br ${colors.overlay} backdrop-blur-[2px]`}
        />
      )}
      ...
      <div className={`relative h-full flex flex-col justify-between ${padding} text-white`}>
        ...
        <div className="space-y-3">
          <h3
            className={`${titleSize} font-bold line-clamp-3 text-white drop-shadow-lg leading-tight`}
          >
            {title}
          </h3>
