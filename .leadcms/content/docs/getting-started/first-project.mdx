...
updatedAt: '2025-11-11T07:50:30.838676Z'
title: First Project
description: >-
...
Before connecting to your CMS instance, verify that the site runs using the local content. Instead of the default `pnpm run dev` command (which fetches remote content and will fail without a API KEY), initialize the LeadCMS SDK and use the Next.js CLI directly:
...
<Steps>
 <Step title="Run the init command">
    ...
    ```
    This command prompts you for your LeadCMS URL (e.g. https://localhost:8080 since you'll be running LeadCMS locally). Skip authentication and use read‑only mode for now, but to push content you'll need an API key, which can be added later on to the .env.
  </Step>
