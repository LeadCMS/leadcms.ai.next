...
export interface WhyReasonCardProps {
  icon?: string
  title: string
  children: React.ReactNode
}

export const WhySectionTitle: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const textContent = getTextContent(children)
  return <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl">{textContent}</h2>
}

export const WhySectionDescription: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const textContent = getTextContent(children)
  return <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">{textContent}</p>
}

export const WhyReasonCard: React.FC<WhyReasonCardProps> = ({ icon, title, children }) => {
...
export const WhySection: React.FC<WhySectionProps> = ({ title, description, children, variant = "muted" }) => {
  ...
  const reasons = childArray.filter(child =>
    React.isValidElement(child) && child.type === WhyReasonCard
  )

  const backgroundClasses: Record<NonNullable<WhySectionProps["variant"]>, string> = {
    muted: "bg-muted",
  ...
  return (
    <section className={cn("w-full py-12 md:py-24 lg:py-32", backgroundClasses[variant])} id="why">
